...
func TestFilterSkipErrors(t *testing.T) {
	...
	t.Run("when does not contains skip error", func(t *testing.T) {
		...
		act := safeguard.FilterSkipErrors(errs, safeguard.SkipErr(errors.New("does not contains")))

		require.Equal(t, errs, act)
	})

	t.Run("when contains simple skip error", func(t *testing.T) {
		t.Parallel()

		skip := safeguard.SkipErr(errors.New("skip error"))

		err1, err2 := errors.New("1"), errors.New("2")
		errs := []error{err1, skip, err2}
		act := safeguard.FilterSkipErrors(errs, skip)

		require.Equal(t, []error{err1, err2}, act)
	})
	...
	t.Run("when contains join skip error", func(t *testing.T) {
		...
		skip := safeguard.SkipErr(errors.New("skip error"))
		joined := errors.Join(skip, errors.New("not skip"))

		err1, err2 := errors.New("1"), errors.New("2")
		errs := []error{err1, joined, err2}
		act := safeguard.FilterSkipErrors(errs, skip)
